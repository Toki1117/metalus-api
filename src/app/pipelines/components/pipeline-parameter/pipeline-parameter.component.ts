...
import { IPipeline, IPipelineStepParam } from '../../../core/models/pipelines.model';
...
@Component({
	...
	styleUrls: [
		'./pipeline-parameter.component.css'
	]
})
export class PipelineParameterComponent {
	...
	openEditor(id: number) {
		...
		if (this.stepGroup && this._parameter.name == 'pipelineMappings') {
			...
			if (
				this._parameter.name == 'pipelineId' ||
				(this._parameter.name == 'pipeline' && inputData.type === 'pipeline')
			) {
