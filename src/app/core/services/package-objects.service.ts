...
import { IPackageObject, IPackageObjectResponse, IPackageObjectsResponse } from '../interfaces/package-objects.model';
...
export class PackageObjectsService {
	...
	getPackageObjects(): Observable<IPackageObject[]> {
		return this.http.get<IPackageObjectsResponse>(`/api/v1/package-objects`, { observe: 'response' }).pipe(
			map((response) => {
	...
	updatePackageObjects(pkgObjs: IPackageObject[]): Observable<IPackageObject[]> {
		...
		return this.http
			.post<IPackageObjectResponse>('/api/v1/package-objects', bulkPackageObjects, { observe: 'response' })
